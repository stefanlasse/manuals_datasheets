...
\node (enable) [menu_style, below of=step_unit] {
\begin{minipage}[h]{5cm}
\LCD{2}{16}|Turn output|
|on / off|
\end{minipage}
 \hfill
\begin{minipage}[h]{\textWidth}
Enable or disable motor\\ outputs.
\end{minipage}
};

\node (internal_prog) [menu_style, below of=enable] {
\begin{minipage}[h]{5cm}
...
\draw [arrow] (step_unit) -- (enable);
\draw [arrow] (enable) -- (internal_prog);
...
\begin{center}
  \LCD{2}{16}|{rarrow}on      on|
             |{rarrow}on      off|
\end{center}

\textbf{Note:} During startup the \productName ~will automatically detect if a stepper motor is connected to an output. If no stepper motor is connected to a certain output it will be automatically disabled.
...
\node (fullrotation) [menu_style, below of=gearratio] {
\begin{minipage}[h]{5cm}
\LCD{2}{16}|Set steps per|
|full rotation|
\end{minipage}
 \hfill
\begin{minipage}[h]{\textWidth}
Set steps per full rotation of the stepper motors.
\end{minipage}
};

\node (substep) [menu_style, below of=fullrotation] {
\begin{minipage}[h]{5cm}
\LCD{2}{16}|Change motor|
|substep|
\end{minipage}
 \hfill
\begin{minipage}[h]{\textWidth}
Edit the number of substeps\\ used for microstepping.
\end{minipage}
};
...
\node (decay) [menu_style, below of=motor_curr] {
\begin{minipage}[h]{5cm}
\LCD{2}{16}|Set current|
|decay mode|
\end{minipage}
 \hfill
\begin{minipage}[h]{\textWidth}
Set the mode used for\\ current decay.
\end{minipage}
};

\node (step_wait_time) [menu_style, below of=decay] {
\begin{minipage}[h]{5cm}
\LCD{2}{16}|Change step|
|wait time|
\end{minipage}
...
\draw [arrow] (gearratio) -- (fullrotation);
\draw [arrow] (fullrotation) -- (substep);
\draw [arrow] (substep) -- (motor_curr);
\draw [arrow] (motor_curr) -- (decay);
\draw [arrow] (decay) -- (step_wait_time);
\draw [arrow] (step_wait_time) -- (save);
\draw [arrow] (save) -- (load);
\draw [arrow] (load.east) -- +(1,0) |- (gearratio.east);
\end{tikzpicture}
...
\label{menu_gear_ratio}
\index{gear ratio}
This menu allows the user to adjust the gear ratio according to the specifications of the connected stage. See~\ref{example_gear_ratio} for an example of how to determine the correct gear ratio of a stage.
In this menu there is also a fast mode available (please refer to  \ref{menu_motor_pos} for details about the fast mode).
\index{gear ratio}
\begin{center}
  \LCD{2}{16}|{rarrow}1.000   3.000|
             | 3.000   3.000|
\end{center}
...
\begin{center}
  \LCD{2}{16}|{rarrow}3.00ms {rarrow}5.25ms|
